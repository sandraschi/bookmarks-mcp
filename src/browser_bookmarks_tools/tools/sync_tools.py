...
async def sync_bookmarks(
    ...
    if src_res.get("status") not in ("success", "ok"):
        return {
            ...
            "fix": (f"verify {source_browser} installed | check bookmarks file exists | review source_message"),
        }
                ...
                failures.append(
                    {
                        ...
                        "fix": (f"check {target_browser} bookmarks file permissions | verify disk space"),
                    }
                )
