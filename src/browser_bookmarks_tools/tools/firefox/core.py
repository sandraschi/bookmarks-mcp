...
async def check_firefox_bruteforce_access() -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "error", "message": f"Brute force access check failed: {e!s}"}
...
async def get_firefox_profiles() -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Error reading Firefox profiles: {e!s}",
            "safety_check": safety_check,
        }


# DEPRECATED: Use firefox_profiles(operation='create_firefox_profile') instead
async def create_firefox_profile(profile_name: str, template_profile: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "error", "message": f"Failed to create profile: {e!s}"}
...
async def delete_firefox_profile(profile_name: str, confirm_deletion: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "error", "message": f"Failed to delete profile: {e!s}"}
    ...
    try:
        ...
        if not bookmarks:
            return {
                "status": "warning",
                "message": (
                    f"Profile '{profile_name}' created but no bookmarks were loaded from source '{source_type}'"
                ),
                "profile_name": profile_name,
                "profile_path": str(profile_path),
                "bookmarks_loaded": 0,
        ...
        return {"status": "error", "message": f"Failed to create loaded profile: {e!s}"}
...
async def _fetch_bookmarks_from_source(
    ...
    try:
        ...
        logger = logging.getLogger(__name__)
        logger.error(f"Error fetching bookmarks from {source_type}: {e!s}")
        return []
...
async def _fetch_from_web_list(config: dict[str, Any], max_bookmarks: int) -> list[dict[str, str]]:
    ...
    try:
        ...
        logger = logging.getLogger(__name__)
        logger.error(f"Error scraping web list from {url}: {e!s}")
        return []
...
async def _fetch_from_github_awesome(config: dict[str, Any], max_bookmarks: int) -> list[dict[str, str]]:
    ...
    try:
        ...
        logger = logging.getLogger(__name__)
        logger.error(f"Error fetching GitHub awesome repos for {topic}: {e!s}")
        return []
    ...
    try:
        ...
        for bookmark in bookmarks:
            try:
                ...
                result = await add_bookmark(url=bookmark["url"], title=bookmark["title"], profile_name=profile_name)
        ...
        logger = logging.getLogger(__name__)
        logger.error(f"Error populating profile with bookmarks: {e!s}")
        return 0
    ...
    try:
        ...
        if invalid_presets:
            ...
            return {
                "status": "error",
                "message": (
                    f"Invalid presets: {', '.join(invalid_presets)}. Available: {', '.join(available_presets)}"
                ),
                "available_presets": available_presets,
            }
        ...
        return {"status": "error", "message": f"Failed to create portmanteau profile: {e!s}"}
