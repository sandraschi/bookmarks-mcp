...
- search_bookmarks()  firefox_bookmarks(operation='search_bookmarks')
- find_duplicates()  firefox_bookmarks(operation='find_duplicates')
"""
...
async def find_duplicates(
    ...
    try:
        ...
        if by == "url":
            ...
            GROUP BY p.url
            HAVING COUNT(*) >= ?
            ORDER BY count DESC, p.url
            """
        else:
            query = """
            SELECT b.title, GROUP_CONCAT(b.id) as ids, COUNT(*) as count
            FROM moz_bookmarks b
            JOIN moz_places p ON b.fk = p.id
            WHERE b.type = 1
            GROUP BY b.title
        ...
        return {"status": "error", "message": f"Error finding duplicates: {e!s}"}
    finally:
        if "db" in locals():
            db.close()
