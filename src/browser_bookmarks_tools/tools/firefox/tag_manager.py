...
async def list_tags(profile_path: str | None = None, min_count: int = 0, max_count: int = 1000) -> dict[str, Any]:
    ...
    if not dry_run:
        for bookmark_id in bookmarks_to_update:
            ...
            if set(new_tags) != set(current_tags):
                ...
                changes.append({"bookmark_id": bookmark_id, "old_tags": current_tags, "new_tags": new_tags})
...
# DEPRECATED: Use firefox_tagging portmanteau instead
async def clean_up_tags(min_count: int = 1, profile_path: str | None = None, dry_run: bool = True) -> dict[str, Any]:
