...
- get_bookmark()  firefox_bookmarks(operation='get_bookmark')
- add_bookmark()  firefox_bookmarks(operation='add_bookmark')
"""
...
async def add_bookmark(
    ...
    try:
        ...
        if place:
            ...
            # Insert new place
            cursor.execute(
                "INSERT INTO moz_places (url, title, rev_host, hidden, typed, frecency) VALUES (?, ?, ?, 0, 1, -1)",
                (
                    url,
                    title,
        ...
        if description:
            cursor.execute("UPDATE moz_places SET description = ? WHERE id = ?", (description, place_id))
