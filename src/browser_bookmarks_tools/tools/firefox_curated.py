...
from browser_bookmarks_tools.operation_types import FirefoxCuratedOperation
from browser_bookmarks_tools.tool_responses import unknown_operation_response
from browser_bookmarks_tools.tools.firefox.curated_sources import (
    get_curated_source,
...
async def _get_curated_source(source_name: str | None, include_metadata: bool) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"Failed to get curated source: {e!s}",
            "source_name": source_name,
            "source": {},
        }


async def _list_curated_sources(category: str | None, include_metadata: bool, limit: int) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"Failed to list curated sources: {e!s}",
            "category": category,
            "sources": [],
            "count": 0,
        }
...
async def _list_curated_bookmark_sources(category: str | None, include_metadata: bool, limit: int) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"Failed to list curated bookmark sources: {e!s}",
            "category": category,
            "bookmark_sources": [],
            "count": 0,
        }
