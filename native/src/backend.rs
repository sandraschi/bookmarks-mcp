...
const BACKEND_NAME: &str = "bookmarks-mcp-backend.exe";
const BACKEND_PORT: u16 = 10803;

fn dev_backend_path() -> Option<PathBuf> {
    if !cfg!(debug_assertions) { return None; }
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("binaries")
        .join("bookmarks-mcp-backend-x86_64-pc-windows-msvc.exe");
...
fn resolve_bundled_backend(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    if let Ok(path) = app.path().resolve("resources/bookmarks-mcp-backend.exe", BaseDirectory::Resource) {
        tried.push(path.display().to_string());
        if path.exists() { return Ok(path); }
    }
...
pub fn spawn_backend(app: AppHandle, state: &BackendProcess) -> Result<String, String> {
    ...
    if let Some(out) = stdout {
        let h = app.clone(); thread::spawn(move || { let r = BufReader::new(out); for line in r.lines().map_while(Result::ok) { log_line(&h, &line); } });
    }
