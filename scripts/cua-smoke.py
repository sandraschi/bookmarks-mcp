...
PROCESS_NAMES = cfg(
    ...
    [OPERATOR_EXE.replace(".exe", ""), f"{OPERATOR_EXE.replace('.exe', '').replace('-native', '')}-backend"],
)
...
def log(m):
    print(f"  [cua] {m}", flush=True)


def fatal(m):
    print(f"  [cua] FATAL: {m}", flush=True)
    sys.exit(1)
...
def silent_install(inst):
    ...
    if r.returncode != 0:
        fatal(f"Install failed: {r.returncode}")
    time.sleep(2)
